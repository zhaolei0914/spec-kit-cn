...
def download_and_extract_template(project_path: Path, ai_assistant: str, script_type: str, is_current_dir: bool = False, *, verbose: bool = True, tracker: StepTracker | None = None, client: httpx.Client = None, debug: bool = False, github_token: str = None, local_package: str = None) -> Path:
    """Download the latest release and extract it to create a new project.
    Returns project_path. Uses tracker if provided (with keys: fetch, download, extract, cleanup)
    If local_package is provided, use that zip file instead of downloading from GitHub.
    """
    ...
    if local_package:
        ...
        if not local_path.exists():
            raise FileNotFoundError(f"本地包不存在: {local_package}")
        if not local_path.suffix == '.zip':
            raise ValueError(f"本地包必须是 .zip 文件: {local_package}")
        zip_path = local_path
        if tracker:
            tracker.skip("fetch", "使用本地包")
            tracker.add("download", "Download template")
            tracker.skip("download", f"本地: {local_path.name}")
        elif verbose:
            console.print(f"[cyan]使用本地包:[/cyan] {local_path}")
    else:
        if tracker:
    ...
    try:
        ...
        # 只有远程下载的包才需要清理，本地包不删除
        if not local_package and zip_path.exists():
            zip_path.unlink()
            if tracker:
...
@app.command()
def init(
    project_name: str = typer.Argument(None, help="项目目录名称（使用 --here 时可选，或使用 '.' 表示当前目录）"),
    ai_assistant: str = typer.Option(None, "--ai", help="AI 助手: claude, gemini, copilot, cursor-agent, qwen, opencode, codex, windsurf, kilocode, auggie, codebuddy, amp, shai, q, bob, or jules"),
    script_type: str = typer.Option(None, "--script", help="脚本类型: sh 或 ps"),
    ignore_agent_tools: bool = typer.Option(False, "--ignore-agent-tools", help="跳过 AI agent 工具检查"),
    no_git: bool = typer.Option(False, "--no-git", help="跳过 git 仓库初始化"),
    here: bool = typer.Option(False, "--here", help="在当前目录初始化项目"),
    force: bool = typer.Option(False, "--force", help="强制合并/覆盖（跳过确认）"),
    skip_tls: bool = typer.Option(False, "--skip-tls", help="跳过 SSL/TLS 验证（不推荐）"),
    debug: bool = typer.Option(False, "--debug", help="显示详细诊断输出"),
    github_token: str = typer.Option(None, "--github-token", help="GitHub token（或设置 GH_TOKEN/GITHUB_TOKEN 环境变量）"),
    local_package: str = typer.Option(None, "--local-package", help="使用本地 zip 包而非从 GitHub 下载"),
):
    """
    ...
    if here:
        ...
        if project_path.exists():
            # 目录已存在时，改为合并模式（类似 --here）
            here = True
            existing_items = list(project_path.iterdir())
            if existing_items:
                console.print(f"[yellow]注意:[/yellow] 目录 '{project_name}' 已存在（{len(existing_items)} 个项目）")
                console.print("[yellow]模板文件将与现有内容合并，可能会覆盖部分文件[/yellow]")
                if force:
                    console.print("[cyan]--force 已指定: 跳过确认，继续合并[/cyan]")
                else:
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            download_and_extract_template(project_path, selected_ai, selected_script, here, verbose=False, tracker=tracker, client=local_client, debug=debug, github_token=github_token, local_package=local_package)
